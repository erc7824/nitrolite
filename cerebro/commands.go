...
func (o *Operator) checkpoint(ctx context.Context, asset string) {
	...
	// Debug: show signer info
	skPK, _, _, skErr := o.store.GetSessionKey()
	if skErr == nil {
		skSigner, _ := sign.NewEthereumRawSigner(skPK)
		fmt.Printf("DEBUG: State signer = SessionKey (0x01), address: %s\n", skSigner.PublicKey().Address().String())
	} else {
		fmt.Println("DEBUG: State signer = Default wallet (0x00)")
	}

	// Debug: show channel info
	channel, chErr := o.client.GetHomeChannel(ctx, wallet, asset)
	if chErr == nil {
		fmt.Printf("DEBUG: Channel ID: %s\n", channel.ChannelID)
		fmt.Printf("DEBUG: Channel ApprovedSigValidators: %s\n", channel.ApprovedSigValidators)
		fmt.Printf("DEBUG: Channel Status: %d\n", channel.Status)
	} else {
		fmt.Printf("DEBUG: Failed to get channel: %v\n", chErr)
	}

	// Debug: show state signatures and verify locally
	state, stErr := o.client.GetLatestState(ctx, wallet, asset, true)
	if stErr == nil {
		fmt.Printf("DEBUG: State version: %d, transition: %s\n", state.Version, state.Transition.Type.String())
		if state.UserSig != nil {
			sig := *state.UserSig
			if len(sig) >= 4 {
				fmt.Printf("DEBUG: UserSig prefix (type byte): %s\n", sig[:4])
			}
			fmt.Printf("DEBUG: UserSig length: %d chars\n", len(sig))
		}
		if state.NodeSig != nil {
			sig := *state.NodeSig
			if len(sig) >= 4 {
				fmt.Printf("DEBUG: NodeSig prefix (type byte): %s\n", sig[:4])
			}
			fmt.Printf("DEBUG: NodeSig length: %d chars\n", len(sig))
		}

		// Re-sign the state locally to see what our current signer produces
		reSigned, reSignErr := o.client.SignState(state)
		if reSignErr == nil {
			fmt.Printf("DEBUG: Re-signed UserSig length: %d chars\n", len(reSigned))
			if len(reSigned) >= 4 {
				fmt.Printf("DEBUG: Re-signed prefix (type byte): %s\n", reSigned[:4])
			}
			if state.UserSig != nil && reSigned == *state.UserSig {
				fmt.Println("DEBUG: Re-signed sig MATCHES stored UserSig")
			} else {
				fmt.Println("DEBUG: Re-signed sig DIFFERS from stored UserSig")
				fmt.Printf("DEBUG: Re-signed: %s\n", reSigned)
			}
		} else {
			fmt.Printf("DEBUG: Re-sign failed: %v\n", reSignErr)
		}

		// Verify stored session key metadata matches what's in the signature
		if skErr == nil {
			_, storedMH, storedAuthSig, _ := o.store.GetSessionKey()
			fmt.Printf("DEBUG: Stored metadataHash: %s\n", storedMH)
			fmt.Printf("DEBUG: Stored authSig len:  %d chars\n", len(storedAuthSig))

			// Verify the auth sig recovers to the wallet address
			storedMHHash := common.HexToHash(storedMH)
			skSigner, _ := sign.NewEthereumRawSigner(skPK)
			skAddr := skSigner.PublicKey().Address().String()
			packedAuth, packErr := core.PackChannelKeyStateV1(skAddr, storedMHHash)
			if packErr == nil {
				authSigBytes, _ := hexutil.Decode(storedAuthSig)
				recoverer, _ := sign.NewAddressRecoverer(sign.TypeEthereumMsg)
				recoveredWallet, recErr := recoverer.RecoverAddress(packedAuth, authSigBytes)
				if recErr != nil {
					fmt.Printf("DEBUG: Auth sig recovery FAILED: %v\n", recErr)
				} else {
					fmt.Printf("DEBUG: Auth sig recovered wallet: %s\n", recoveredWallet.String())
					if strings.EqualFold(recoveredWallet.String(), wallet) {
						fmt.Println("DEBUG: Auth sig wallet match OK")
					} else {
						fmt.Printf("DEBUG: Auth sig wallet MISMATCH! expected=%s\n", wallet)
					}
				}
			} else {
				fmt.Printf("DEBUG: Failed to pack auth data: %v\n", packErr)
			}
		}
	} else {
		fmt.Printf("DEBUG: Failed to get state: %v\n", stErr)
	}
