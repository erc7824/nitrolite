...
func (h *Handler) handleOperateIntent(
	...
	for participant, assets := range currentAllocations {
		for asset, currentAmount := range assets {
			...
			if !diff.IsZero() {
				if err := tx.RecordLedgerEntry(appStateUpd.AppSessionID, asset, diff); err != nil {
	...
	for participant, assets := range currentAllocations {
		for asset, currentAmount := range assets {
			...
			if incomingAmount.LessThan(currentAmount) {
				...
				if err := tx.RecordLedgerEntry(appStateUpd.AppSessionID, asset, withdrawAmount.Neg()); err != nil {
...
func (h *Handler) handleCloseIntent(
	...
	for participant, assets := range currentAllocations {
		for asset, amount := range assets {
			...
			if err := tx.RecordLedgerEntry(appStateUpd.AppSessionID, asset, amount.Neg()); err != nil {
