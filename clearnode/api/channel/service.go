...
func (s *ChannelService) RequestClose(params *CloseChannelParams, rpcSigners map[string]struct{}, logger Logger) (ChannelOperationResponse, error) {
	...
	if err = checkChallengedChannels(s.db, channel.UserWallet); err != nil {
	...
	_, ok := rpcSigners[channel.UserWallet]
	if !ok {
		return ChannelOperationResponse{}, RPCErrorf("invalid signature")
	}

	asset, ok := s.assetsCfg.GetAssetTokenByAddressAndChainID(channel.Token, channel.BlockchainID)
	if !ok {
		logger.Error("failed to find asset for an existing channel", "token", channel.Token, "chainID", channel.BlockchainID)
		return ChannelOperationResponse{}, RPCErrorf("failed to find asset for token %s on chain %d", channel.Token, channel.BlockchainID)
	}

	ledger := GetWalletLedger(s.db, common.HexToAddress(channel.UserWallet))
	balance, err := ledger.Balance(NewAccountID(channel.UserWallet), asset.Symbol)
	if err != nil {
