...
func NewBalanceNotification(wallet string, database *gorm.DB) *Notification {
	balances, _ := db.GetWalletLedger(database, common.HexToAddress(wallet)).GetBalances(db.NewAccountID(wallet))
	return &Notification{
		userID:    wallet,
...
func NewChannelNotification(channel db.Channel) *Notification {
	return &Notification{
