...
func NewRPCNode(signer sign.Signer, logger log.Logger) *RPCNode {
	return &RPCNode{
		upgrader: websocket.Upgrader{
			ReadBufferSize:  1024,
...
read_loop:
	for {
		...
		msg := RPCMessage{Req: &rpc.Payload{}}
...
type SendRPCMessageFunc func(method string, params rpc.Params)
...
func (c *RPCContext) Succeed(method string, params rpc.Params) {
	c.Message.Res = &rpc.Payload{
...
func prepareRawRPCResponse(signer *Signer, data *rpc.Payload) ([]byte, error) {
	if data == nil {
...
func prepareRawNotification(signer *Signer, method string, params rpc.Params) ([]byte, error) {
	if params == nil {
		params = struct{}{}
	}
...
func (wn *RPCNode) Notify(userID, method string, params rpc.Params) {
...
func (wn *RPCNode) getSendMessageFunc(conn *RPCConnection) SendRPCMessageFunc {
	return func(method string, params rpc.Params) {
		message, err := prepareRawNotification(wn.signer, method, params)
		if err != nil {
