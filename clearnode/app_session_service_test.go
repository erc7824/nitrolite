...
func TestAppSessionService_GetAppSessions(t *testing.T) {
	db, cleanup := setupTestDB(t)
	defer cleanup()

	addrA := "0xParticipantA"
	addrB := "0xParticipantB"
	addrC := "0xParticipantC"

	service := NewAppSessionService(db)
	sessions := []AppSession{
		{SessionID: "s1", ParticipantWallets: []string{addrA, addrB}, Status: ChannelStatusOpen},
		{SessionID: "s2", ParticipantWallets: []string{addrA, addrC}, Status: ChannelStatusClosed},
		{SessionID: "s3", ParticipantWallets: []string{addrB, addrC}, Status: ChannelStatusOpen},
	}
	for _, s := range sessions {
		require.NoError(t, db.Create(&s).Error)
	}

	t.Run("GetByParticipant", func(t *testing.T) {
		res, err := service.GetAppSessions(addrA, "")
		require.NoError(t, err)
		assert.Len(t, res, 2)
	})

	t.Run("GetByStatus", func(t *testing.T) {
		res, err := service.GetAppSessions("", string(ChannelStatusOpen))
		require.NoError(t, err)
		assert.Len(t, res, 2)
	})

	t.Run("GetByParticipantAndStatus", func(t *testing.T) {
		res, err := service.GetAppSessions(addrA, string(ChannelStatusOpen))
		require.NoError(t, err)
		assert.Len(t, res, 1)
		assert.Equal(t, "s1", res[0].SessionID)
	})

	t.Run("GetAll", func(t *testing.T) {
		res, err := service.GetAppSessions("", "")
		require.NoError(t, err)
		assert.Len(t, res, 3)
	})
}
