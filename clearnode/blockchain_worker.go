...
func (w *BlockchainWorker) processAction(_ context.Context, action database.BlockchainAction) bool {
	...
	if err != nil {
		...
		w.metrics.IncBlockchainAction(state.Asset, w.blockchainID, fmt.Sprintf("%d", action.Type), false)
		return false
	}
	...
	w.metrics.IncBlockchainAction(state.Asset, w.blockchainID, fmt.Sprintf("%d", action.Type), true)
