...
func (s *ChannelService) RequestResize(logger Logger, params *ResizeChannelParams, rpcSigners map[string]struct{}) (ResizeChannelResponse, error) {
	...
	if rawBalance.Cmp(newChannelAmount) < 0 {
		return ResizeChannelResponse{}, RPCErrorf("insufficient unified balance for channel %s: required %s, available %s", channel.ChannelID, newChannelAmount.String(), rawBalance.String())
	}
	newChannelAmount.Add(newChannelAmount, params.ResizeAmount)
	if newChannelAmount.Cmp(big.NewInt(0)) < 0 {
		return ResizeChannelResponse{}, RPCErrorf("new channel amount must be positive: %s", newChannelAmount.String())
	}
