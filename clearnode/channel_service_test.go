	...
	ch := Channel{
		...
		State: UnsignedState{
			...
			Allocations: []Allocation{
				{Participant: participant, TokenAddress: token, RawAmount: rawAmount},
				{Participant: "broker", TokenAddress: token, RawAmount: decimal.Zero},
			},
		},
	}
...
func TestChannelService(t *testing.T) {
	...
	t.Run("RequestCreate_Success", func(t *testing.T) {
		...
		assert.Equal(t, StateIntentInitialize, response.State.Intent, "Intent should be INITIALIZE (1)")
