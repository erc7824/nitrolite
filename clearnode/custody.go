...
func (c *Custody) handleJoined(logger Logger, ev *nitrolite.CustodyJoined) {
	...
	err := c.db.Transaction(func(tx *gorm.DB) error {
		...
		_, err = RecordTransaction(tx, TransactionTypeDeposit, channelID, channel.Wallet, asset.Symbol, tokenAmount)
		if err != nil {
	...
	err := c.db.Transaction(func(tx *gorm.DB) error {
		...
		if resizeAmount.Cmp(big.NewInt(0)) != 0 {
			...
			if amount.IsPositive() || amount.IsZero() {
				...
				_, err = RecordTransaction(tx, TransactionTypeDeposit, channelID, channel.Wallet, asset.Symbol, amount)
				if err != nil {
					return fmt.Errorf("failed to record transaction: %w", err)
				}
			} else {
				...
				_, err = RecordTransaction(tx, TransactionTypeWithdrawal, channel.Wallet, channelID, asset.Symbol, amount)
				if err != nil {
					return fmt.Errorf("failed to record transaction: %w", err)
				}
...
func (c *Custody) handleClosed(logger Logger, ev *nitrolite.CustodyClosed) {
	...
	err := c.db.Transaction(func(tx *gorm.DB) error {
		...
		_, err = RecordTransaction(tx, TransactionTypeWithdrawal, channel.Wallet, channelID, asset.Symbol, tokenAmount)
		if err != nil {
			return fmt.Errorf("failed to record transaction: %w", err)
		}
		// 2. Withdraw from the channel account.
