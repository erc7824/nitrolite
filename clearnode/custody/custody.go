...
type UnsignedState = db.UnsignedState
...
type Custody struct {
	...
	blockStep          uint64
	wsNotifier         *api.WSNotifier
	logger             log.Logger
}

// NewCustody initializes the Ethereum client and custody contract wrapper.
func NewCustody(signer sign.Signer, db *gorm.DB, wsNotifier *api.WSNotifier, blockchain memory.BlockchainConfig, assetsCfg *memory.AssetsConfig, logger log.Logger) (*Custody, error) {
	...
	return &Custody{
		...
		logger:             logger.WithName("custody").With("chainID", blockchain.ID).With("custodyAddress", blockchain.ContractAddresses.Custody),
	}, nil
}

// ListenEvents initializes event listening for the custody contract
func (c *Custody) ListenEvents(ctx context.Context) {
	ev, err := db.GetLatestContractEvent(c.db, c.custodyAddr.Hex(), c.chainID)
	if err != nil {
...
func (c *Custody) HandleBlockChainEvent(ctx context.Context, l types.Log) {
	ctx = log.SetContextLogger(ctx, c.logger)
...
func (c *Custody) handleCreated(logger Logger, ev *nitrolite.CustodyCreated) {
	logger = logger.WithKV("event", "Created")
	...
	c.wsNotifier.Notify(NewBalanceNotification(ch.UserWallet, c.db))
}

func (c *Custody) handleChallenged(logger Logger, ev *nitrolite.CustodyChallenged) {
	logger = logger.WithKV("event", "Challenged")
...
func (c *Custody) handleClosed(logger Logger, ev *nitrolite.CustodyClosed) {
	logger = logger.WithKV("event", "Closed")
	channelID := common.Hash(ev.ChannelId).Hex()
	...
	err := c.db.Transaction(func(tx *gorm.DB) error {
		...
		walletAddress := common.HexToAddress(channel.UserWallet)
		channelAccountID := NewAccountID(channelID)
		walletAccountID := NewAccountID(channel.UserWallet)
		...
		channel.OnChainStateVersion = ev.FinalState.Version.Uint64()
	...
	c.wsNotifier.Notify(
		NewBalanceNotification(channel.UserWallet, c.db),
