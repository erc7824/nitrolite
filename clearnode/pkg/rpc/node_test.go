...
func TestNode(t *testing.T) {
	...
	onConnectCounts := 0
	node.OnConnect(func(send SendResponseFunc) {
		mu.Lock()
		defer mu.Unlock()
		...
		if err != nil {
			logger.Error("failed to create params", "error", err)
			return
		}
	...
	node.OnAuthenticated(func(userID string, send SendResponseFunc) {
		mu.Lock()
		defer mu.Unlock()
	...
	createDummyHandler := func(method string) func(c *Context) {
	...
	node.Handle(authMethod, func(c *Context) {
	...
	// Receive message
	receive := func(t *testing.T) Response {
		var res Response
	...
	sendAndReceive := func(t *testing.T, RequestID uint64, method string, params Params) Response {
		if params == nil {
			params = Params{}
		}
		// Create request
		reqData := NewPayload(RequestID, method, params)
	...
	t.Run("root handler", func(t *testing.T) {
		...
		err := resp.Res.Params.Translate(&params)
		require.NoError(t, err)
	...
	t.Run("auth handler", func(t *testing.T) {
		...
		err := resp.Res.Params.Translate(&params1)
		require.NoError(t, err)

		assert.Equal(t, authenticatedUserID, params1[paramsUserIDKey]) // authenticated user ID
		assert.True(t, onMessageSentSignal.await())                    // on message sent signal
		mu.Unlock()
		...
		err = resp.Res.Params.Translate(&params2)
		require.NoError(t, err)

		assert.Equal(t, authenticatedUserID, params2[paramsUserIDKey]) // authenticated user ID
		assert.True(t, onMessageSentSignal.await())                    // on message sent signal
	...
	t.Run("invalid message format", func(t *testing.T) {
		...
		assert.Contains(t, res.Error().Error(), "invalid message format")
		assert.True(t, onMessageSentSignal.await()) // on message sent signal
	})
