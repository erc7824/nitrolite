...
func (r *RPCRouter) HandleAuthVerify(c *RPCContext) {
	...
	if len(req.Params) < 1 {
		c.Fail(nil, "invalid parameters: expected at least 1 parameter")
		return
	}

	paramsJSON, err := json.Marshal(req.Params[0])
	if err != nil {
		c.Fail(nil, fmt.Sprintf("invalid parameters format: %s", err.Error()))
		return
	}

	var authParams AuthVerifyParams
	if err := json.Unmarshal(paramsJSON, &authParams); err != nil {
		c.Fail(nil, fmt.Sprintf("failed to parse auth parameters: %s", err.Error()))
		return
	}
