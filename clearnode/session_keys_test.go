...
func TestSessionKey(t *testing.T) {
	...
	err := AddSessionKey(db, walletAddress, sessionSignerAddress, appName, scope, allowances, expirationTime)
	...
	assert.Equal(t, "session", sk.SignerType)
	...
	err = json.Unmarshal([]byte(*sk.SpendingCap), &retrievedSpendingCap)
	require.NoError(t, err)
	...
	allSigners, err := GetAllSignersForWallet(db, walletAddress)
	require.NoError(t, err)
	assert.Len(t, allSigners, 2)

	var sessionCount, custodyCount int
	for _, s := range allSigners {
		if s.SignerType == "session" {
			sessionCount++
			assert.NotNil(t, s.ApplicationName)
		} else if s.SignerType == "custody" {
			custodyCount++
			assert.Nil(t, s.ApplicationName)
		}
	}
	assert.Equal(t, 1, sessionCount)
	assert.Equal(t, 1, custodyCount)
}
...
func TestSessionKeyMultipleKeys(t *testing.T) {
	...
	for _, sk := range sessionKeys {
		err := AddSessionKey(db, walletAddress, sk.signerAddress, sk.appName, sk.scope, []Allowance{}, time.Now().Add(time.Hour))
		require.NoError(t, err)
	}
...
func TestSessionKeyActiveKeys(t *testing.T) {
	...
	// Add an expired session key
	err = AddSessionKey(db, walletAddress, "0xexpired123", "ExpiredApp", "view", []Allowance{}, time.Now().Add(-1*time.Hour))
	require.NoError(t, err)
	...
	activeKeys, err := GetActiveSessionKeysByWallet(db, walletAddress)
	require.NoError(t, err)
...
func TestSessionKeySpendingValidation(t *testing.T) {
	...
	retrievedKey, err := GetSessionKeyByKey(db, sessionKeyAddress)
	require.NoError(t, err)
...
func TestOneSessionKeyPerApp(t *testing.T) {
	...
	err = AddSessionKey(db, walletAddress, sessionKey1, appName, "trade", allowances, expiration)
	require.NoError(t, err)
	...
	assert.Equal(t, appName, *sessionKeys[0].ApplicationName)

	// Add second session key for the SAME app - should invalidate the first one
	err = AddSessionKey(db, walletAddress, sessionKey2, appName, "trade", allowances, expiration)
	require.NoError(t, err)
