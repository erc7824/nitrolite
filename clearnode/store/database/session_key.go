...
var (
	ErrSessionKeyExistsAndExpired = errors.New("session key already exists but is expired")
	ErrSignerUsedForAnotherWallet = errors.New("signer is already in use for another wallet")
)
...
func AddSessionKey(db *gorm.DB, walletAddress, address, applicationName, scope string, allowances []Allowance, expirationTime time.Time) error {
	...
	if isExpired(expirationTime) {
		return errors.New("expiration time must be set and in the future")
	}
