...
contract CustodyTest is Test {
    ...
    function createInitialState(address[2] memory participants, uint256 counter) internal returns (State memory) {
        // Create allocations for both participants
        Allocation memory hostAllocation = Allocation({
            destination: participants[0],
            token: address(token),
            amount: DEPOSIT_AMOUNT
        });
        
        Allocation memory guestAllocation = Allocation({
        ...
        // Encode Counter adjudicator data
    ...
    function signState(Channel memory chan, State memory state, uint256 privateKey) internal view returns (Signature memory) {
    ...
    function test_ChannelChallenge() public {
        ...
        Counter.CounterApp memory challengeCounterApp = Counter.CounterApp({
            counter: 2,
            target: TARGET,
            version: 1
        });
        ...
        Counter.CounterApp memory counterChallengeApp = Counter.CounterApp({
            counter: 3,
            target: TARGET,
            version: 2
        });
    ...
    function test_Checkpoint() public {
        ...
        Counter.CounterApp memory checkpointCounterApp = Counter.CounterApp({
            counter: 2,
            target: TARGET,
            version: 1
        });
    ...
    function test_DepositAndWithdraw() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                Custody.InsufficientBalance.selector, 
                DEPOSIT_AMOUNT / 2, 
                DEPOSIT_AMOUNT
            )
        );
