...
contract CounterTest is Test {
    ...
    function test_adjudicate_initialState_valid() public view {
        // Create initial state with target 10
        State memory state = _createInitialState(10);

        // Add both signatures
        state.sigs = new Signature[](2);
        state.sigs[HOST] = _signState(state, hostPrivateKey);
        state.sigs[GUEST] = _signState(state, guestPrivateKey);

        // Verify that the state is valid
        State[] memory emptyProofs = new State[](0);
        bool valid = counter.adjudicate(channel, state, emptyProofs);
        assertTrue(valid, "Valid initial state should be accepted");
    }

    function test_adjudicate_initialState_revert_whenSingleSignature() public view {
        // Create initial state with target 10
        State memory state = _createInitialState(10);

        // Add only host signature
        state.sigs = new Signature[](1);
        state.sigs[HOST] = _signState(state, hostPrivateKey);

        // Verify that the state is rejected
        State[] memory emptyProofs = new State[](0);
        bool valid = counter.adjudicate(channel, state, emptyProofs);
        assertFalse(valid, "Initial state with only one signature should be rejected");
    }

    function test_adjudicate_initialState_revert_whenNonZeroVersion() public view {
        // Create initial state with target 10 but version 1
        State memory state = _createInitialState(10);
        state.version = 1;
        ...
        State[] memory emptyProofs = new State[](0);
        bool valid = counter.adjudicate(channel, state, emptyProofs);
        assertFalse(valid, "Initial state with non-zero version should be rejected");
    }

    function test_adjudicate_initialState_revert_whenInvalidSignature() public {
        // Create initial state with target 10
        State memory state = _createInitialState(10);

        // Add signatures, but corrupt the host signature
        state.sigs = new Signature[](2);
        Signature memory hostSig = _signState(state, hostPrivateKey);
        hostSig.r = bytes32(0); // Corrupt the signature
        state.sigs[HOST] = hostSig;
        state.sigs[GUEST] = _signState(state, guestPrivateKey);

        // Verify that the signature verification fails
        State[] memory emptyProofs = new State[](0);
        vm.expectRevert(ECDSA.ECDSAInvalidSignature.selector);
        counter.adjudicate(channel, state, emptyProofs);
    }
