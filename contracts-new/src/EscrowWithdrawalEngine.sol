...
library EscrowWithdrawalEngine {
    ...
    function _calculateInitiateEffects(
        ...
        require(candidate.nonHomeState.nodeAllocation == withdrawalAmount, "node allocation must equal withdrawal amount");
        require(candidate.nonHomeState.nodeNetFlow == withdrawalAmount.toInt256(), "node net flow must equal withdrawal amount");
    ...
    function _calculateFinalizeEffects(
        ...
        require(candidate.homeState.userAllocation < ctx.initState.homeState.userAllocation, "home user allocation must decrease");
        ...
        require((-homeNodeNfDelta).toUint256() == withdrawalAmount, "home node net flow delta must equal withdrawal amount");
    ...
    function _validateInvariants(
        ...
        if (candidate.intent == StateIntent.INITIATE_ESCROW_WITHDRAWAL) {
            ...
            require((-effects.userFundsDelta).toUint256() == ctx.lockedAmount, "released amount must equal locked amount");
        }
    }
}
