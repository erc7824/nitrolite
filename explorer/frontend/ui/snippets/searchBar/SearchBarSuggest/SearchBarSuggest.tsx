...
import SearchBarSuggestChannel from './SearchBarSuggestChannel';
        ...
        {resultCategories.map((cat, index) => {
          return (
            <Element name={ `cat_${ cat.id }` } key={ cat.id }>
              ...
              {cat.id === 'channel' && itemsGroups[cat.id]?.map((item, index) => (
                <SearchBarSuggestChannel
                  key={index}
                  data={item.data}
              ...
              {cat.id !== 'app' && cat.id !== 'channel' && itemsGroups[cat.id]?.map((item, index) => (
