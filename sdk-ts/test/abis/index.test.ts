...
describe('ABIs', () => {
  describe('CustodyAbi', () => {
    it('should export a valid ABI', () => {
      ...
      const hasFunctions = CustodyAbi.some(item => 
        item.type === 'function' && 
        ['open', 'close', 'challenge', 'checkpoint', 'reclaim'].includes(item.name as string)
      );
  ...
  describe('AdjudicatorAbi', () => {
    it('should export a valid ABI', () => {
      ...
      const hasAdjudicate = AdjudicatorAbi.some(item => 
        item.type === 'function' && item.name === 'adjudicate'
      );
  ...
  describe('Erc20Abi', () => {
    it('should export a valid ABI', () => {
      ...
      // Check for expected functions
      const hasFunctions = Erc20Abi.some(item => 
        item.type === 'function' && 
        ['approve', 'transfer', 'transferFrom', 'balanceOf', 'allowance'].includes(item.name as string)
      );
  ...
  describe('defaultAbiConfig', () => {
    ...
    it('should include warning about placeholders', () => {
      ...
      const moduleContent = require('fs').readFileSync(
        require('path').resolve(__dirname, '../../src/abis/index.ts'), 
        'utf8'
      );
