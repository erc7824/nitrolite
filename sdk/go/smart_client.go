...
// SmartClient provides high-level methods for interacting with Clearnode.
...
// NewSmartClient creates a new SmartClient and establishes a connection to the Clearnode server.
...
//
// Returns:
//   - Configured SmartClient ready for high-level operations
...
//	    sdk.WithBlockchainRPC(80002, "https://polygon-amoy.alchemy.com/v2/KEY"),
//	)
func NewSmartClient(wsURL string, signer sign.Signer, opts ...Option) (*SmartClient, error) {
	// Create base client
...
func (c *SmartClient) initializeBlockchainClient(ctx context.Context, chainID uint64) error {
...
func (c *SmartClient) getTokenAddress(ctx context.Context, blockchainID uint64, asset string) (string, error) {
...
func (c *SmartClient) getContractAddress(ctx context.Context, blockchainID uint64) (string, error) {
	nodeConfig, err := c.GetConfig(ctx)
	if err != nil {
		return "", fmt.Errorf("failed to get node config: %w", err)
	}
...
func (c *SmartClient) getNodeAddress(ctx context.Context) (string, error) {
	nodeConfig, err := c.GetConfig(ctx)
	if err != nil {
		return "", fmt.Errorf("failed to get node config: %w", err)
	}
...
func (c *SmartClient) SignState(state *core.State) (string, error) {
	if state == nil {
...
func (c *SmartClient) GetUserAddress() string {
...
func (c *SmartClient) signAndSubmitState(ctx context.Context, state *core.State) (string, error) {
	// Sign state
...
func (c *SmartClient) waitForTransaction(ctx context.Context, chainID uint64, txHash string) error {
...
func (c *SmartClient) Transfer(ctx context.Context, recipientWallet string, asset string, amount decimal.Decimal) (string, error) {
...
func (c *SmartClient) Deposit(ctx context.Context, blockchainID uint64, asset string, amount decimal.Decimal) (string, error) {
	...
	if err != nil || state.HomeChannelID == nil {
		...
		voidState.HomeChannelID = &homeChannelID

		// Apply deposit transition
		_, err = voidState.ApplyHomeDepositTransition(amount)
		if err != nil {
			return "", fmt.Errorf("failed to apply deposit transition: %w", err)
		}

		// Calculate state ID
		voidState.ID = core.GetStateID(voidState.UserWallet, voidState.Asset, voidState.Epoch, voidState.Version)

		// Sign state
		sig, err := c.SignState(voidState)
		if err != nil {
			return "", fmt.Errorf("failed to sign state: %w", err)
		}
		voidState.UserSig = &sig

		// Request channel creation from node
		nodeSig, err := c.RequestChannelCreation(ctx, *voidState, channelDef)
		if err != nil {
			return "", fmt.Errorf("failed to request channel creation: %w", err)
		}
		voidState.NodeSig = &nodeSig

		// Create channel on blockchain
		txHash, err := blockchainClient.Create(channelDef, *voidState)
		if err != nil {
...
func (c *SmartClient) Withdraw(ctx context.Context, blockchainID uint64, asset string, amount decimal.Decimal) (string, error) {
	userWallet := c.GetUserAddress()
