  ...
  for (const file of tsFiles) {
    try {
      ...
      if (match) {
        ...
        const paramMatches = jsdocContent.matchAll(
          /\*\s*@param\s+(\w+)\s+(.+?)(?:\n|$)/g
        );
        ...
        const returnsMatch = jsdocContent.match(
          /\*\s*@returns\s+(.+?)(?:\n|$)/
        );
  ...
  try {
    ...
    for (const file of solFiles) {
      try {
        ...
        const singleCommentRegex = new RegExp(
...
function isUsefulContractContext(context: string): boolean {
  ...
  const unhelpfulCount = unhelpfulPatterns.filter((pattern) =>
    lowerContext.includes(pattern.toLowerCase())
  ).length;
  ...
  return (
    hasGoodPattern ||
    (context.length > 30 && !lowerContext.includes("solidity"))
  );
}
        ...
        while ((match = functionCallRegex.exec(content)) !== null) {
          ...
          if (callLine && callLine.includes(functionName)) {
            ...
            const contextLines = lines.slice(
              Math.max(0, callLineIndex - 2),
              callLineIndex + 3
            );
            const contextBlock = contextLines.join("\n").trim();

            if (
              contextBlock.length > 10 &&
              !examples.find((ex) => ex.includes(callLine.trim()))
            ) {
 ...
 */
function generateEnhancedParameterDocs(
  params: any[],
  ...
  const paramDescriptions = functionName
    ? analyzeParameterUsage(functionName, params)
    : {};
...
function generateEnhancedFunctionDocs(functions: FunctionInfo[]): string {
  ...
  functions.forEach((func) => {
    ...
    if (jsdocInfo?.description && jsdocInfo.description.length > 20) {
      ...
      description = inferFunctionPurpose(
        func.name,
        func.inputs,
        func.outputs,
    ...
    docs += "**Parameters:**\n";
    docs +=
      generateEnhancedParameterDocs(func.inputs, jsdocInfo, func.name) + "\n\n";
...
function main() {
  ...
  try {
    ...
    console.log(
      `Documentation generation complete! Generated docs for ${contracts.length} contracts.`
    );
