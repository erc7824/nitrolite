...
class ExampleTutorialGenerator {
  ...
  constructor(
    ...
    outputDir: string = "./docs/tutorials"
  ) {
  ...
  async generateAllTutorials(): Promise<void> {
    ...
    console.log(
      `Found ${exampleDirs.length} example projects: ${exampleDirs.join(", ")}`
    );
  ...
  private async generateTutorialForProject(projectName: string): Promise<void> {
    ...
    const tutorial = await this.parseTutorialFromProject(
      projectPath,
      projectName
    );
  ...
  private async parseTutorialFromProject(
    ...
    return {
      title:
        tutorialMeta.title ||
        `${projectName.charAt(0).toUpperCase() + projectName.slice(1)} Tutorial`,
      description:
        tutorialMeta.description ||
        `Learn how to build ${projectName} with Nitrolite`,
      ...
      metadata: {
        ...
        concepts: tutorialMeta.concepts || [
          "State Channels",
   ...
   */
  private parseFileForDocumentation(
    content: string,
    filename: string
  ): TutorialSection[] {
    const sections: TutorialSection[] = [];
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (!inDocBlock && sections.length > 0) {
        ...
        if (
          lastSection.type === "step" &&
          !lastSection.metadata?.codeExtracted
        ) {
  ...
  private generateMarkdownFromTutorial(tutorial: TutorialStructure): string {
    ...
    const architectureSections = tutorial.sections.filter(
      (s) => s.type === "architecture"
    );
    const conceptSections = tutorial.sections.filter(
      (s) => s.type === "concept"
    );
  ...
  private renderSection(section: TutorialSection, stepCount?: number): string {
    ...
    switch (section.type) {
      case "step":
        ...
        const stepAnchorId = this.generateAnchorId(
          `step ${stepCount} ${stepTitle}`
        );
