import { Address, Hex, encodeAbiParameters, Abi } from 'viem';
import { AppDataTypes, AppLogic } from '../../types';
...
export function createCustomChannel<T extends Hex | unknown = unknown>(
    ...
    try {
        if (params.adjudicatorAddress) {
            ...
            adjudicatorAddress = client.getAdjudicatorAddress(
                params.adjudicatorType
            );
        } else {
    ...
    if (params.adjudicatorAbi && params.adjudicatorType) {
        client.registerAdjudicatorAbi(
            params.adjudicatorType,
            params.adjudicatorAbi
        );
    }
